...
def _record_field(raw: str, label: str) -> str:
    """발송기록 한 필드를 세 표기 관례로 뽑는다 — 헤딩형(옛 관례) · 목록형(합성
    데모) · 표형(실제 `g5-제안메일제작발송` 발송 모드 산출, 2026-08-14 실물 실행으로
    처음 확인됨: `web/parsers.py`의 `send_record_field`와 같은 이유로 같은 세 관례를 받는다."""
    m = (
        re.search(rf"###\s*{label}\s*\n+([^\n]+)", raw)
        or re.search(rf"\*\*{label}\*\*[:：]?\s*([^\n]+)", raw)
        or re.search(rf"\|\s*{label}\s*\|\s*([^\|\n]+)\|", raw)
    )
    return m.group(1).strip() if m else ""


def _match_send_record(subject: str, sender: str) -> str:
    """어느 발송 건에 대한 회신인지 대조한다 — 제목만으로는 안 한다.

    제목만 보고 고르면 두 가지가 겹친다: ① 같은 세그먼트는 대표 문안을 공유해
    여러 기업이 **동일한 제목**을 받는다 — 후보가 여럿이면 파일명 정렬 순으로
    아무거나 골랐었다(실제로는 무작위 선택과 같다). ② 제목은 수신자라면 누구나
    아는 값이라, 제3자가 같은 제목으로 보낸 메일도 그 "아무 후보"에 잘못
    귀속될 수 있다(2026-08-14 검토로 발견). 그래서 **제목이 같은 후보 중에서도
    발신자 주소가 그 발송 기록의 수신 주소와 일치하는 것만** 채택한다 —
    제목만 같고 주소가 다른 건 "이 회신이 우리가 보낸 메일에 대한 답인가"
    자체(`_is_reply_to_us`)와는 무관하게(회신 처리·수신거부 판정은 그대로 진행),
    "어느 기업 건으로 귀속시킬지"만 비워 둔다(빈 문자열 — 기존 `-미확정` 관례로
    이어진다). 주소 위조(발신자 헤더 자체를 속이는 것)까지는 이 함수가 못 막는다
    — SPF/DKIM 검증이 필요한 별개 문제라 `docs/90`에 남겨 둔다."""
    ...
    for path in sorted(SENT_DIR.glob("*.md")):
        try:
            ...
            continue
        if _normalize_subject(_record_field(raw, "제목")) != target:
            continue
        to_field = _record_field(raw, "수신\\s*주소")
        to_addr = (email.utils.parseaddr(to_field)[1] or to_field).lower()
        if to_addr and to_addr == sender_addr:
    ...
    if SENT_DIR.is_dir():
        for path in SENT_DIR.glob("*.md"):
            try:
                ...
                continue
            subj = _record_field(raw, "제목")
            if subj:
                subjects.add(_normalize_subject(subj))
