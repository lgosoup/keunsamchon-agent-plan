...
## H9는 실제 구현, 나머지 10개는 더미 모듈이 있다 (2026-08-10)

11개 중 **H9(criteria 수정 감지)만 Claude Code 자체의 Hook 메커니즘으로 실제 구현 가능**하다 — criteria 파일 수정은 이 실행 환경(Claude Code) 자신이 관측할 수 있는 사건이기 때문이다. 나머지 10개는 메일 도착·발송 서비스 응답·시간 경과처럼 이 실행 환경 밖의 사건이라 **감지·스케줄링 자체는 별도 플랫폼(구현은 팀원이 진행 중)이 있어야** 한다.
