...
| `hook/` | 자동 트리거 11개(H1~H11) 명세 — 언제·무슨 신호로·어느 스킬을 호출하는가. **H9는 Claude Code 자체 메커니즘으로 실제 구현 예시(`settings.json`+`hook_h9_criteria_review.sh`)까지 있고**, 나머지 10개는 이 실행 환경 밖의 사건이라 구현은 그 플랫폼에서 사람이 한다(`hook/README.md` 참조) |
